...
abstract contract Factory__MapTokensSimulation_Base is Migration {
  ...
  function _cheatWeightGovernor(IBridgeManager manager, address gov) internal virtual {
    ...
    bytes32 newGovWeight = bytes32(uint256(totalWeight));

    vm.store(address(manager), $, newGovWeight);
