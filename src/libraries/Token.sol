...
library Token {
  ...
  function validate(Info memory _info) internal pure {
    if (!((_info.erc == Standard.ERC20 && _info.quantity > 0 && _info.id == 0) || (_info.erc == Standard.ERC721 && _info.quantity == 0))) {
   ...
   *
   */
  function handleAssetTransfer(Info memory _info, address payable _to, address _token, IWETH _wrappedNativeToken) internal {
    bool _success;
