...
contract MockBridgeReward is BridgeReward {
  function calcRewardAndCheckSlashedStatus(
    ...
    return _calcRewardAndCheckSlashedStatus(isValidTrackingResponse, numBridgeOperators, rewardPerPeriod, ballot, totalBallot, period, slashUntilPeriod);
  }
  ...
  function isValidBridgeTrackingResponse(uint256 totalBallot, uint256 totalVote, uint256[] memory ballots) external pure returns (bool valid) {
    return _isValidBridgeTrackingResponse(totalBallot, totalVote, ballots);
  }

  function shouldShareEqually(uint256 totalBallot, uint256 totalVote, uint256[] memory ballots) external returns (bool shareEqually) {
